...
/**
 * @author xav Important, dans le fichier de config Hibernate, vous êtes en
 *         « update » <property name="hbm2ddl.auto">update</property>
 */
public class Test20_requetage {
	public static void main(String[] args) {
		...
		List<Entraineur> entraineurs = (List<Entraineur>) session.createQuery("from metier.Entraineur").list();// REQUETE
																												// HQL
		...
		System.out.println("\n\n");
		// Affichez tous les adh�rents
		List<Adherent> adherents = (List<Adherent>) session.createQuery("from metier.Adherent").list();// REQUETE
																												// HQL
		...
		// TODO
		if(s_a.equals(s_b)) {
			System.out.println("s_a == s_b, ce sont bien les mêmes objets.");
		}
